...
in the latter case (MixedCase BlackBerry) it returns info primarily about the device
(inferring what you meant from the case).
"""
...
def test_ddg_api_case_sensitive_query(ddg, url_params, logger, query):
    ...
    with pytest.allure.step("Send request: q={!r}".format(query)):
        ...
        data_original = resp.json()
        # logger.debug("\nOriginal response:\n{!r}".format(data_original))
        pytest.allure.attach(
            "original_response.json",
            json.dumps(data_original, indent=4),
            pytest.allure.attach_type.JSON
        )
    ...
    with pytest.allure.step("Send request: q={!r}".format(query_lower)):
        ...
        resp = ddg.get(params=url_params)
        resp.raise_for_status()
        data_lower = resp.json()
        # logger.debug("\nResponse for lower case query:\n{!r}".format(data_lower))
        pytest.allure.attach(
            "lower_case_response.json",
            json.dumps(data_lower, indent=4),
            pytest.allure.attach_type.JSON
        )
